...
//
// The Firestore rule now requires sign-in to read (see README) — that's
// what stops the raw vote data from being world-readable. One side effect:
// anyone browsing before they sign in can't load live counts, so this is
// (re)attached once a user signs in, and shows a friendly placeholder
// until then instead of leaving the leaderboard blank.
...
function subscribeToVotes() {
  ...
  unsubscribeVotes = db.collection("votes").onSnapshot(
    ...
    () => {
      // Expected for anyone not yet signed in — reads require auth now.
      leaderboardSlotsEl.innerHTML =
        '<p class="leaderboard-empty">Sign in on any photo to see live results.</p>';
    }
  );
}
